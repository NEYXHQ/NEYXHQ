...
import (
	...
	"github.com/Palo_Alt0/vanilla-NEYX/testutil/network"
	"github.com/Palo_Alt0/vanilla-NEYX/x/token/client/cli"
)
...
func TestDeleteToken(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		tc := tc
		t.Run(tc.desc, func(t *testing.T) {
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteToken(), append([]string{tc.id}, tc.args...))
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
